...
GRAPHTypeOK == 
 ...
 Messages ==
  ...
  \cup [type : {"INCOMPATIBLE", "committed", "aborted", "prepared"}, tn: transactionNumbers, src : NODES, dst : NODES, leadingEdge: SUBSET transactionNumbers, shard: Shard, shards: SUBSET Shard]
...
\*      operatedNodes == 
  ...
  ConflictTx(txInfo, TxsSequence) ==
    LET
        f(x) == {transactions[x][j].sourceVertex : j \in 1..Len(transactions[x])}
        operatedNodes == { f(x) : x \in setFromSeq(TxsSequence) }
        txInfoOps == {transactions[txInfo][op].sourceVertex : op \in 1..Len(transactions[txInfo])}
        hasIntersection ==  \E x \in operatedNodes : txInfoOps \cap x # {}
    IN
        hasIntersection
  ...
  PreparedConflictCatchup(r, TxsSequence) ==
    {tx \in localTransactionHistory[r]["prepared"] : ConflictTx(tx, TxsSequence)}
      
  
  CoordinatedTxs(r, TxsSequence, shardInfo) ==
    {tx \in PreparedConflictCatchup(r, TxsSequence) : rmState[tx, r, shardInfo] = "leader"}
...
\*  /\ msgs' = [msgs EXCEPT ![r][s] = Append(msgs[r][s], "aaa")]
  ...
  LET
    ...
    constructAbortMsg(msgOneType, msgTwoType) ==
               [ m \in {[type |-> msgOneType, 
                    tn |->tnInfo, 
                    dependency |-> localTransactionHistory[s]["leadingEdge"], 
                    src |-> s, 
    ...
    prepareAndVoteAbort == constructAbortMsg("prepare", "aborted")
...
\*                                                                   ![s]["prepared"] = localTransactionHistory[s]["prepared"] \ {tnInfo}]
     /\ localTransactionHistory' =  [ localTransactionHistory   EXCEPT ![s]["prepared"] = localTransactionHistory[s]["prepared"] \ {tnInfo}
                                                                           ,![s]["committed"] =  localTransactionHistory[s]["committed"] \cup {tnInfo}
                                                                           ,![s]["leadingEdge"] = (localTransactionHistory[s]["leadingEdge"] \ localTransactionalGraph[s][tnInfo]) \union {tnInfo}] 
...
\*    /\ test' = test + 1
\*    /\ UNCHANGED <<newCoordinator,lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, localTransactionHistory, 
...
\*       /\ test' = test + 1
\*       /\ UNCHANGED <<newCoordinator,lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, 
  ...
  IN  
      ...
      THEN
        ...
        THEN 
            ...
            /\ test' = test + 1
            /\ UNCHANGED << lostMsgCount, failedNodesCount, failedNodes, clientRequests, localNodesGraph, localTransactionHistory, localTransactionalGraph, pendingTransactions, rmState, catchUpID>>
...
\*            /\ localTransactionalGraph' =  [localTransactionalGraph EXCEPT ![r] = @ @@ [i \in {tnInfo} |-> (localTransactionHistory[r]["leadingEdge"] \ Parents(commonElements, localTransactionalGraph[r])) \union commonElements ]]
            ...
            /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, 
                 clientRequests, pendingTransactions, rmState, catchUpID>>
          ...
          ELSE
            ...
            /\ test' = test + 1
            /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, 
                localNodesGraph, clientRequests, pendingTransactions, rmState, localTransactionHistory, localTransactionalGraph, catchUpID>>
      ELSE
         /\ test' = test + 1
\*          /\ ~MsgExist([type |-> "aborted", tn |-> tnInfo, src |-> r, dst |-> s, shards |-> shardsInfo, shard |-> shardInfo, leadingEdge |-> localTransactionHistory[r]["leadingEdge"]], msgs)
  ...
  (*************************************************************************)
   ...
   /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, rmState, msgs, transactionNumbers, clientRequests, localNodesGraph
      ,pendingTransactions, localTransactionalGraph, catchUpID>>
...
\*  /\ tnState' = [tnState EXCEPT ![tnInfo, r] = "committed"]
  /\ test' = test + 1
  /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, rmState, clientRequests, 
      pendingTransactions, localTransactionalGraph, catchUpID>>
  ELSE
    /\ SendCatchUp(r, shardInfo, s , tnInfo)
    /\ test' = test + 1
    /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, rmState, clientRequests, 
      pendingTransactions, localTransactionalGraph, localNodesGraph, localTransactionHistory>>
    
\*  /\ UNCHANGED <<newCoordinator,tmState, 
 ...
 TopoSort(graph, remaining, depth) ==
  ...
  THEN
      ...
      ELSE
        ...
        IN AppendSetToSeq(ready,TopoSort(graph, rest, depth + 1))
  ELSE
    FALSE
...
\*                   , msgs)
  ...
  LET 
\*    txnSet == {transactions[i]: i \in setFromSeq(transactionSequence)}
\*    hasConflict == \E t \in txnSet : ConflictDetect(-1, r, txnSet)
    ...
    conflictCoordinatedTxs == CoordinatedTxs(r, transactionSequence, 1)
    AbortMsgSet == {[type |-> "abort", 
                tn |-> tn, 
                src |-> r, 
                dst |-> -1, 
                operations |-> transactions[tn],
                shards |-> transactionShards[tn],
                shard |-> 1
                ] 
              : tn \in conflictCoordinatedTxs}
    AbortCatchUpSet  == UNION{{[type |-> "finCatUp", src |-> r, dst |-> s, ID |-> requestID]}, AbortMsgSet}
    constructAbortMsg == [ m \in AbortCatchUpSet |-> 1 ]
     
     
  IN  
     IF conflictCoordinatedTxs # {} 
     THEN
     /\ ~(setFromSeq(transactionSequence) \subseteq localTransactionHistory[r]["committed"])
     ...
     /\ localTransactionHistory' = [localTransactionHistory EXCEPT ![r]["leadingEdge"] = @ \union Leaf,
                                                                   ![r]["aborted"] = (@ \ DOMAIN subGraph) \union conflictCoordinatedTxs,
                                                                   ![r]["committed"] = newCommitted,
                                                                   ![r]["prepared"] = @ \ setFromSeq(transactionSequence)]
     /\ localNodesGraph' = [localNodesGraph EXCEPT ![r] = ApplyOpsquence(transactionSequence, r, localNodesGraph[r])]
      
     /\ ~MsgExist([type |-> "finCatUp", src |-> r, dst |-> s, ID |-> requestID], msgs)
     /\ msgs' = msgs @@ constructAbortMsg          
     ELSE
     /\ ~(setFromSeq(transactionSequence) \subseteq localTransactionHistory[r]["committed"])
     /\ localTransactionalGraph' = [localTransactionalGraph EXCEPT ![r] = MergeGraphs(localTransactionalGraph[r], subGraph)]
     /\ localTransactionHistory' = [localTransactionHistory EXCEPT ![r]["leadingEdge"] = @ \union Leaf,
  ...
  (*******************************************************************************************************************************)
    ...
    /\ \E MS \in Quorum(msg.shard):    
        LET 
            ...
            constructAbortCatchMsg == [m \in { [type|-> "catchUp", src |-> r, ID |-> tnInfo, shard |-> msg.shard, leadingEdge |-> localTransactionHistory[r]["leadingEdge"], leader |-> r],
                      [type |-> "abort", 
                       tn|-> tnInfo, 
                       src |-> r, 
                       dst |-> -1, 
...
\*            /\ LeaderFowardSingleShardCommit(msg.tn, r, msg.shards, msg.shard)
            ...
            /\ test' = test + 1
            /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, rmState, clientRequests, localTransactionalGraph,
                         pendingTransactions, clientRequests, pendingTransactions, rmState, catchUpID>>
            ELSE
            /\ test' = test + 1
...
\*            /\ LeaderSendAbort(msg.tn, r, msg.shards, msg.shard)
            /\ ~MsgExist([type|-> "catchUp", src |-> r, ID |-> tnInfo, shard |-> msg.shard, leadingEdge |-> localTransactionHistory[r]["leadingEdge"], leader |-> r], msgs)
            /\ msgs' = msgs @@ constructAbortCatchMsg
            /\ catchUpID' = [catchUpID EXCEPT ![r] = @ + 1]
\*            /\ SendCatchUp(r,  msg.shard, r, tnInfo)
            /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, clientRequests, localNodesGraph, localTransactionHistory, localTransactionalGraph, pendingTransactions, rmState>>
  ...
  newLeaderHandleAbort(tnInfo, r, msg) ==
  (*********************************************************************************)
...
\*            /\ r \in newCoordinator
            /\ test' = test + 1
     /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, rmState, clientRequests, localTransactionHistory, localNodesGraph, localTransactionalGraph,
                         pendingTransactions, clientRequests, localNodesGraph, localTransactionHistory, pendingTransactions, catchUpID>>           
...
\*      /\ msgsShards' = msgsShards @@ prepareMsg
   ...
   IN 
      ...
      /\ test' = test + 1
      /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, clientRequests, localNodesGraph, pendingTransactions, catchUpID>>
...
\*        \/ tnShardState[msg.tn, r] = "primarySendCommitted"  \*  primary leader should be aboe to recv the commit msg even itself sent out committed
   /\ LeaderSendCommit(msg.tn, r, msg.shards, msg.shard)
   /\ test' = test + 1
   /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, clientRequests, localTransactionalGraph, pendingTransactions, rmState, catchUpID>>
   ...
   /\ LeaderSendAbort(msg.tn, r, msg.shards, msg.shard)
   /\ test' = test + 1
   /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, clientRequests, localNodesGraph, localTransactionHistory, pendingTransactions, rmState, localTransactionalGraph, catchUpID>>
...
\*   /\ test' = test + 1
\*   /\ UNCHANGED <<newCoordinator,lostMsgCount, failedNodesCount, failedNodes, clientRequests, localNodesGraph, localTransactionHistory, msgs, pendingTransactions, localTransactionalGraph, rmState, catchUpID>>
...
\*   /\ test' = test + 1
\*   /\ UNCHANGED <<newCoordinator,lostMsgCount, failedNodesCount, failedNodes, clientRequests, localNodesGraph, localTransactionHistory, msgs, pendingTransactions, rmState, localTransactionalGraph, catchUpID>> 
\*      
...
\*        
\*       /\ UNCHANGED <<newCoordinator,transactionNumbers, localTransactionHistory, 
...
\*       \/tnState[msg.tn, r] = "sendPrepared"   \*   the node sent out send prepared should still able to recv its own prepare message 
  ...
  RecvNewLeaderAborted(msg) == 
   /\ msg.type = "aborted"
   /\ msg.dst \in ShardNodeMapping[msg.shard]
   /\ newLeaderHandleAbort(msg.tn, msg.dst, msg)
...
\*  /\ newCoordinator' = newCoordinator \union {r}
  ...
  /\ UNCHANGED << catchUpID, clientRequests, failedNodes, failedNodesCount, localNodesGraph, localTransactionHistory, localTransactionalGraph, lostMsgCount, pendingTransactions, test>>
   ...
   ClientRequest(i) == 
    ...
    /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, msgs, localTransactionHistory, 
...
\*        /\ PrimaryLeaderSendPrepares(clientRequest, i, transactions[clientRequest], transactionShards[clientRequest])
        /\ InterposedCoordinatorSendPrepares(clientRequest, i, transactions[clientRequest], {1}, 1)
        /\ clientRequests' = [clientRequests EXCEPT ![i] = Tail(clientRequests[i])]
        
       /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, transactionNumbers, 
        localNodesGraph  , pendingTransactions, rmState, test, catchUpID>>
...
\*    /\ (i = 1 /\ nextExecuteTx = 1) \/ (i = 1 /\ nextExecuteTx = 2) \/ (i = 3 /\ nextExecuteTx = 3)
  ...
  RecvCatchUpResponse(r, message) == 
    ...
    /\ test' = test + 1
    /\ UNCHANGED <<lostMsgCount, failedNodesCount, failedNodes, catchUpID, clientRequests, pendingTransactions, rmState>>
  ...
  FailNodes(r) ==
    ...
    /\ test' = test + 1
    /\ UNCHANGED <<lostMsgCount, rmState,msgs, clientRequests, localTransactionHistory, localTransactionalGraph,                        
            localNodesGraph, pendingTransactions, catchUpID>>
\*    
\*   ShardsMSGLost(m) == 
...
\*   /\ test' = test + 1
\*   /\ UNCHANGED <<newCoordinator,rmState,msgs, clientRequests, localTransactionHistory, localTransactionalGraph,                        
\*            localNodesGraph, pendingTransactions, failedNodes, failedNodesCount, catchUpID>>
   ...
   /\ lostMsgCount' = lostMsgCount + 1
   /\ test' = test + 1
   /\ UNCHANGED <<rmState, clientRequests, localTransactionHistory, localTransactionalGraph,                        
            localNodesGraph, pendingTransactions, failedNodes, failedNodesCount, catchUpID>>
...
\*        /\ ~(n1 \in failedNodes)
\*        /\ ~(n2 \in failedNodes)
        /\ a \in DOMAIN localTransactionalGraph[n1]
        /\ b \in DOMAIN localTransactionalGraph[n1]
        /\ a \in DOMAIN localTransactionalGraph[n2]
        /\ b \in DOMAIN localTransactionalGraph[n2]
        /\ b \in localTransactionalGraph[n1][a]
     => 
        b \in localTransactionalGraph[n2][a]
  ...
  ConflictingTxNodeCommit ==
  LET
  setOfTx == { transactionNumbers[i] : i \in DOMAIN transactionNumbers }
  IN
  \A tx1, tx2 \in setOfTx :
    \A a, b \in NODES :
      ~(/\ tx1 # tx2
      /\ tx1 \in localTransactionHistory[a]["committed"]
      /\ tx2 \notin localTransactionHistory[a]["committed"]
      /\ tx2 \in localTransactionHistory[a]["prepared"]   
       
      /\ tx2 \in localTransactionHistory[b]["committed"]
      /\ tx1 \notin localTransactionHistory[b]["committed"]
      /\ tx1 \in localTransactionHistory[a]["prepared"]
      /\ tx1 \notin localTransactionalGraph[a][tx2]
      /\ tx2 \notin localTransactionalGraph[a][tx1])
       
  ConflictingTxNodeCommit2 ==
      \A a, b \in NODES :
        ~(/\ 1 \in localTransactionHistory[a]["committed"]
        /\ 2 \in localTransactionHistory[a]["committed"]
        /\ Cardinality(localNodesGraph[a]) = 1
        /\ 2 \in localTransactionHistory[b]["committed"]
        /\ 1 \in localTransactionHistory[b]["prepared"]
        /\ Cardinality(localNodesGraph[b]) = 0)
        
  ConflictingTxNodeCommit3 ==
      \A a, b \in NODES :
        ~(/\ 1 \in localTransactionHistory[a]["committed"]
        /\ 2 \in localTransactionHistory[a]["committed"]
        /\ Cardinality(localNodesGraph[a]) = 1
        /\ 2 \in localTransactionHistory[b]["committed"]
        /\ 1 \in localTransactionHistory[b]["committed"]
        /\ Cardinality(localNodesGraph[b]) = 0)
...
CatchUpInvariants == 
    LET mset(r, shardInfo) == {m \in ValidMessage(msgs) : /\ m.type = "catchUp"
                                      /\ m.shard  = shardInfo
                                      /\ m.src = r
                   }
    IN 
        \A r \in NODES : Cardinality(mset(r, 1)) <= 3



CatchUpResponseInvariants == 
    LET mset(r) == {m \in ValidMessage(msgs) : /\ m.type = "catchUpResponse"
                                      /\ m.src = r
                   }
    IN 
...
PreparedInvariant == 
    LET mset(r) == {m \in ValidMessage(msgs) : /\ m.type = "prepared"
                                      /\ m.src = r
                   }
    IN 
        \A r \in NODES : Cardinality(mset(r)) <= 6
...
GraphEqual1 == \A i, j \in NODES : Cardinality(localNodesGraph[i]) = Cardinality(localNodesGraph[j])
\*
...
vars == <<test, msgs, rmState, clientRequests, localTransactionHistory, localTransactionalGraph,                        
            localNodesGraph, pendingTransactions, failedNodes, failedNodesCount, catchUpID, lostMsgCount>>
